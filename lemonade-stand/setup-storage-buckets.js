...
const supabaseUrl = process.env.VITE_SUPABASE_URL || 'https://hbnrwzumdonidkrakaqk.supabase.co';
...
async function setupBuckets() {
  ...
  try {
    ...
    for (const bucket of bucketsToCreate) {
      ...
      if (bucket.public) {
        console.log(`Setting public access policy for bucket '${bucket.name}'...`);
        
        const { error: policyError } = await supabase.storage.from(bucket.name).getPublicUrl('test');
        
        if (policyError) {
          console.error(`Error setting public policy for bucket '${bucket.name}':`, policyError.message);
        } else {
